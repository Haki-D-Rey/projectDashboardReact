...
const ItemLink: FC<IItemProps> = ({ icon, text, href, isExtended, isMobileOrTablet }) => {
  ...
  return (
    <li>
      <Link
        href={href ? href : ''}
        className={classNames('flex items-center px-6 py-2 pr-2 transition-shadow duration-1000 ease-in-out hover:text-gray-100 ', {
        ...
        <div className="relative flex items-center py-1 text-white">
          <Image className={isCurrentPath ? 'z-40 h-11 w-11 rounded-full border-2 border-tertiary bg-primary p-1' : 'h-8 w-8'} src={icon} alt="" />
          {isExtended || isMobileOrTablet ? (
